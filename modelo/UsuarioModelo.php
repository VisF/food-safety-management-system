...
class UsuarioModelo
{
    ...
    /**
     * Crear nuevo usuario
     * @param array $data Datos del usuario: nombre, apellido, dni, email, password, telefono, domicilio
    ...
    public function crear(array $data)
    {
        ...
        // Verificar unicidad
        $stmt = $this->conexion->prepare('SELECT id FROM usuarios WHERE email = :email OR dni = :dni');
        ...
        $sql = 'INSERT INTO usuarios (nombre, apellido, dni, email, password, telefono, domicilio, activo) VALUES (:nombre, :apellido, :dni, :email, :password, :telefono, :domicilio, 1)';
    ...
    public function obtenerPorEmail(string $email): ?array
    {
        ...
        $stmt = $this->conexion->prepare('SELECT id, nombre, apellido, dni, email, password, telefono, domicilio, activo, fecha_creacion FROM usuarios WHERE email = :email AND activo = 1');
     ...
     * @param int $id ID del usuario
     * @param array $data Datos a actualizar: nombre, apellido, telefono, domicilio, etc.
    ...
    public function cambiarPassword(int $id, string $nuevo_password): bool
    {
        ...
        $stmt = $this->conexion->prepare('UPDATE usuarios SET password = :password WHERE id = :id');
        return (bool)$stmt->execute([':password' => $hash, ':id' => $id]);
    }
